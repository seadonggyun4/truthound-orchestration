...
- Kestra operators defining retry policy for 검증 flows
...
- the team is deciding what the task runner, not the 검증 engine, should own
...
|--------|--------------|------------|
| retries never help | the failure is deterministic 데이터 품질 | remove retries and surface the result directly |
| 검증 saturates the runner | retry counts are too high for expensive tasks | lower attempts and improve gating |
