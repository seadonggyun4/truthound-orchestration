...
Truthound 오케스트레이션 3.x is designed as a first-party compatibility line for Truthound, not as a neutral shell that happens to call several 검증 engines. The architecture is intentionally split between host-native adapters and a shared runtime so that platform ergonomics stay natural while release guarantees stay centralized.

## Design Posture

Truthound 오케스트레이션 3.x is not positioned as a neutral multi-engine shell first. It is the official 오케스트레이션 line for Truthound 3.x, with advanced extension points preserved behind the shared `common` runtime.
...
워크플로우 execution extends that same shared boundary. The 오케스트레이션 layer may normalize 워크플로우 requests, wait or poll on shared flow state, attach artifact refs, and emit compact payloads, but it must not take ownership of 워크플로우 approval, release safety, rollback safety, or business state. See [워크플로우 Pipelines].
