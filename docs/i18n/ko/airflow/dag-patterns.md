...
Truthound works best in Airflow when the 검증 boundary is obvious in the DAG itself: validate before publish, validate before fan-out, or validate after a materialization step. This page documents the patterns that map cleanly to the exported Airflow operators, sensors, and callbacks in this repository.
...
| bounded-memory stream 검증 | the input is large or continuous | `DataQualityStreamOperator` |
...
| one bad dataset stops unrelated branches | 검증 is placed too high in the graph | move checks closer to the branch that owns the data |
