...
Airflow is often the place where Truthound results become operational signals. The Airflow package ships host-native callback and monitoring surfaces so teams can route 검증 failures through the same alerting channels they already use for task and DAG health.
...
- a 검증 failure should send an operational notification
