...
Workflow execution extends that same shared boundary. The orchestration layer may normalize Workflow requests, wait or poll on shared flow state, attach artifact refs, and emit compact payloads, but it must not take ownership of Workflow approval, release safety, rollback safety, or business state. See [Workflow Pipelines].
