...
Airflow is the best fit when Truthound 검증 needs to live inside scheduled DAGs, gate downstream tasks, or reuse existing Airflow connection and alerting patterns.
...
- downstream work must block, branch, or alert based on 검증 results
- existing Airflow connections and alerting policies should remain the operational control plane
- the team wants operators, hooks, and sensors instead of a new 오케스트레이션 abstraction
...
- `truthound-오케스트레이션[airflow]` installed in the Airflow runtime
...
```bash
pip install truthound-오케스트레이션[airflow] "truthound>=3.0,<4.0"
```
...
| move large data through bounded-memory 검증 | `DataQualityStreamOperator` | avoids loading everything into memory |
...
|-----------|------------|
| `DataQualityCheckOperator` | row-level 검증 and pass/fail gating |
