...
`DataQualityResource` is the canonical Dagster boundary for Truthound 오케스트레이션. It is the best choice when you want consistent engine behavior across assets, ops, and asset checks without repeating configuration everywhere.
...
| `profile(...)` | profiling |
| `learn(...)` | rule learning |
| `stream_check(...)` | bounded-memory streaming 검증 |
