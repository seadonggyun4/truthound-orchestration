...
Sensors are for waiting on a quality condition before a downstream task is allowed to continue. In this integration, they stay Airflow-native while leaning on the shared runtime for 검증 semantics.
...
## Truthound — Data Quality 워크플로우Sensor
...
3. 검증 executes against the configured threshold.
