...
- analytics engineers who need `dbt run-operation` 워크플로우s
...
Runs grouped 검증 rules against a relation.
...
Keep options deterministic in CI so that 검증 is reproducible.
