"""SLA Configuration Types for Mage Data Quality Operations.

This module provides immutable configuration types for SLA management
in data quality operations for Mage AI.
...
class SLAConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> SLAConfig:
        ...
        alert_level = data.get("alert_level", "error")
        if isinstance(alert_level, str):
            alert_level = AlertLevel(alert_level.lower())
...
class SLAMetrics:
    ...
    Attributes:
        ...
        block_uuid: Mage block UUID.
        pipeline_uuid: Mage pipeline UUID.
        run_id: Mage run ID.
    ...
    def from_block_result(
        ...
        """Create metrics from BlockResult.

        Args:
            result: BlockResult instance.
            block_uuid: Mage block UUID.
            pipeline_uuid: Mage pipeline UUID.
            run_id: Mage run ID.
        ...
        Returns:
            SLAMetrics: Metrics from result.
        """
        if hasattr(result, "result_dict"):
            return cls.from_check_result(
                result.result_dict,
                block_uuid=block_uuid,
                pipeline_uuid=pipeline_uuid,
                run_id=run_id,
