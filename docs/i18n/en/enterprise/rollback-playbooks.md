...
When the incident involves Workflow-triggered release or rollback flows, treat the shared orchestration layer as the execution and status propagation surface only. Approval and rollback safety still belong to Workflow policy. See [Workflow Pipelines].
