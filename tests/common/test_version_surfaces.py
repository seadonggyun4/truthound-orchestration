...
def test_version_manifest_derives_expected_release_values() -> None:
    ...
    assert config.package_version == "3.0.0"
    assert config.release_line == "3.x"
    assert config.release_tag == "v3.0.0"
    assert config.package_version_tuple == (3, 0, 0)
    assert config.orchestration_self_requirement == ">=3.0.0,<4.0.0"
    assert config.package_series_requirement == ">=3.0.0,<4.0.0"
    assert config.truthound_primary_version == "3.0.0"
    assert config.truthound_install_range == ">=3.0,<4.0"
    assert config.truthound_runtime_requirement == ">=3.0.0,<4.0.0"
...
def test_detects_stale_adapter_tuple_surface(tmp_path: Path) -> None:
    ...
    version_file.write_text(
        version_file.read_text(encoding="utf-8").replace("(3, 0, 0)", "(0, 1, 0)"),
        encoding="utf-8",
    )
...
def test_detects_stale_package_pyproject_version(tmp_path: Path) -> None:
    ...
    pyproject_file.write_text(
        pyproject_file.read_text(encoding="utf-8").replace('version = "3.0.0"', 'version = "3.0.1"', 1),
        encoding="utf-8",
    )
...
def test_detects_stale_dbt_project_version(tmp_path: Path) -> None:
    ...
    dbt_project.write_text(
        dbt_project.read_text(encoding="utf-8").replace("version: '3.0.0'", "version: '3.0.1'", 1),
        encoding="utf-8",
    )
...
def test_detects_stale_support_matrix_truthound_section(tmp_path: Path) -> None:
    ...
    support_matrix.write_text(
        support_matrix.read_text(encoding="utf-8").replace('primary = "3.0.0"', 'primary = "3.0.1"', 1),
        encoding="utf-8",
    )
...
def test_tag_validation_rejects_mismatched_release_tag() -> None:
    ...
    error = sync_version_surfaces.validate_expected_tag(config, "v3.0.1")

    assert error == "tag 'v3.0.1' does not match manifest release tag 'v3.0.0'"
