...
If a 워크플로우 works for local paths but fails for SQL, that usually means the source is normalized correctly and preflight is correctly telling you the missing piece is a real connection contract.
...
- Use `ResolvedDataSource` as the debugging frame of reference when a 워크플로우 behaves differently across hosts.
