...
- 검증 tasks should retry transient failures
- repeated profile or learn operations should reuse cached results
- the team needs clear concurrency boundaries for larger 검증 workloads
...
|--------|--------------|------------|
| failures repeat across retries | the issue is 데이터 품질, not transport | stop retrying and surface the result directly |
