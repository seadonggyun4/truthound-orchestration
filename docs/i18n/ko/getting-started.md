...
```bash
pip install truthound-오케스트레이션 "truthound>=3.0,<4.0"
...
pip install truthound-오케스트레이션[mage] "truthound>=3.0,<4.0"
pip install truthound-오케스트레이션[kestra] "truthound>=3.0,<4.0"
```
...
- omitted rules on the Truthound `check()` path enable auto-schema 검증
...
- you need repeatable CI behavior for dbt or 워크플로우 release gates
