...
Workflow is part of the official orchestration surface for this repository, but it is intentionally positioned as a **pipeline execution layer**, not as a second business-state owner inside the adapters.

Use Workflow surfaces when you need:

- scheduled sync or scheduled validation
- approval-aware merge or release requests
- rollback triggers owned by Workflow policy
- compact shared operation or flow payloads across hosts

Start with [Workflow Pipelines] for the canonical shared contract, then move into the host-specific pages for the native entrypoint on each platform.
