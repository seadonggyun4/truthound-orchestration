...
Truthound 오케스트레이션 supports several host environments, but they solve different problems. This page helps you choose the host that best matches your team, execution model, and operational constraints.

## Start With The Host You Already Operate Well

Truthound does not ask you to adopt a new 오케스트레이션 style just to run 검증. The best default is usually:
...
- dbt if your primary need is warehouse-native 데이터 품질 checks authored in YAML
- Mage if you want lightweight block-level 검증 inside Mage projects
...
|----------------|-----------|-----|
| scheduled 데이터 품질 tasks inside a mature DAG estate | [Airflow](airflow/index.md) | operators, sensors, hooks, and SLA callbacks stay Airflow-native |
...
| 검증 authored directly in SQL project configuration | [dbt](dbt/index.md) | generic tests and macros keep the whole 워크플로우 dbt-native |
...
| dbt | Generic Tests, Macros, Run Operations | SQL 검증 and warehouse enforcement | package management + adapters + CI |
| Mage | Transformers, Sensors, Conditions | simple project-level quality blocks | local project config + Mage runtime |
| Kestra | Python scripts, flow generators, outputs | YAML 워크플로우 generation and script tasks | namespaces + flows + task outputs |
...
- dbt is the smoothest SQL-first path if your 데이터 품질 contract belongs in model YAML.
...
5. Will the first rollout be CI-only, batch-only, or attached to production 워크플로우s?
