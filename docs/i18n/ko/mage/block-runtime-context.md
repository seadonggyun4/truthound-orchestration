...
Truthound's Mage package is organized around Mage-native execution units: transformers, sensors, and conditions. The key runtime bridge is `BlockExecutionContext`, which carries the metadata and behavior needed to keep 검증 consistent across blocks.

## Who This Is For

- Mage authors deciding which block type should own 검증
...
Use a transformer when the block should actively execute 검증:
...
| pipelines become hard to reason about | one block handles 검증 and branching together | separate execution blocks from condition blocks |
