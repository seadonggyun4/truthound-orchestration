...
The dbt adapter validates compiled relations, not local DataFrames or file paths. That is the defining runtime difference between dbt and the Python 오케스트레이션 adapters in this repository.
...
The 검증 target is always a relation that dbt can compile:
