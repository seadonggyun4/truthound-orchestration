"""SLA Monitor for Data Quality Operations.

This module provides SLA monitoring and violation detection for
data quality operations in Apache Airflow.

Example:
    >>> from truthound_airflow.sla import SLAMonitor, SLAConfig
    >>>
    >>> monitor = SLAMonitor(
...
class SLAMonitor:
    ...
    def check(
        ...
        task_id = metrics.task_id or (context or {}).get("task_id")
        dag_id = metrics.dag_id or (context or {}).get("dag_id")
...
class SLARegistry:
    ...
    for different tasks, DAGs, or domains.
