...
- publish structured 검증 results to XCom
- wrap a connection-backed source in a first-class Airflow task
- run Truthound auto-schema 검증 for a fast smoke check

## DataQualityProfileOperator

Use `DataQualityProfileOperator` when the DAG needs descriptive profile output instead of strict pass/fail 검증.
...
## Truthound — Data Quality 워크플로우-Specific Variants
...
- you need to tune timeout behavior for long-running 검증
