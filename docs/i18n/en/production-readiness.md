...
If the rollout depends on Workflow flows such as scheduled sync, release tagging, or rollback triggers, also confirm the orchestration side is treating Workflow as a pipeline execution layer only. Approval, release safety, and rollback safety should remain Workflow-owned. See [Workflow Pipelines].
